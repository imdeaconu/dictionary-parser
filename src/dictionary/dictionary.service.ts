...
export class DictionaryService {
  ...
  async lookupForWordsAndShowResults(wordsToFetch: any[]) {
    this.lookup(wordsToFetch).then((lookupResults) =>
      this.showResults(lookupResults),
    );
  }

  public async lookup(wordsToFetch: WordToFetch[]): Promise<LookupResult> {
    const providerResponse: WordProviderFetchResults =
      await this._wordProvider.fetchSeveralWords(wordsToFetch);
  ...
  public showResults(lookupResult: LookupResult): void {
    ...
    this._dictionaryPresenter.displayNotFoundWords(lookupResult.missing);
  }

  private _addFoundWordsToDictionary(foundWords: WordParams[]) {
    const words: Word[] = foundWords.map((wordParam) => new Word(wordParam));
